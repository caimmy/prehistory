...
def Word2vecIndexAction():
    ...
    if "GET" == request.method:
        posword, negword, book = httpOperation.getParams("posword", "negword", "book")
        err_tips = None
        relwords = None
        if book in current_app.word2vecmodel:
            ...
            try:
                relwords = [m for m in current_app.word2vecmodel[book].most_similar(query_pos, query_neg, topn=100) if len(m[0]) > 1]
        ...
        if book in current_app.word2vecmodel:
            ...
            try:
                relwords = [m for m in current_app.word2vecmodel[book].most_similar(query_pos, query_neg, topn=100) if len(m[0]) > 1]
                httpOperation.changeResponse2Success()
                httpOperation.setData(relwords)
            except KeyError as e:
                httpOperation.setFailureReason(str(e))
                err_tips = "查询词超出词汇表范围！"
            except Exception as e:
                err_tips = str(e)
                httpOperation.setFailureReason(err_tips)
        return httpOperation.JsonResponse()
